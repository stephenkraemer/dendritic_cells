...
beta.df_sampled <- dmrs_cluster_beta %>%
  dplyr::group_by(cluster) %>%
  dplyr::sample_n(size = sampled_number) %>%
  dplyr::arrange(., cluster)

beta.m_sampled <- beta.df_sampled %>%
  ungroup(cluster) %>%
...
max(beta.m_sampled, na.rm=T)
min(beta.m_sampled, na.rm=T)

col_fun = colorRamp2(c(0, 0.5, 1), c("#ffffe5", "#fe9929", "#662506"))
...
h <- Heatmap(beta.m_sampled, 
             ...
             pdf(file=paste0(Sys.Date(), "_DC-Heatmap_", sampled_number, "sampled_beta-value_euclidean_deepSplit=1_minGap=0.1.pdf"), paper="a4r", width = 5, height = 8)
             h + ha.cluster
