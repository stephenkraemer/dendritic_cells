# DMR calling with bsseq - HSC vs CMOP ------------------------------------------------------
# Dr. Sina Stäble
# 30.09.2020
...
# perform DML Test   --------------------------------------------------
  ...
  for(i in 1:length(BSobj.list))
  {
    dmlTest <- DMLtest(BSobj.list[[i]], 
                        ...
                        group2 = c("cmop_1", "cmop_2"),
                        equal.disp = FALSE, smoothing = FALSE, smoothing.span = 500)
    x = str_extract(names(BSobj.list[i]), pattern = "chr[:digit:]+")
    out.dir <- "/icgc/dkfzlsdf/analysis/C010/zimmerms/Collaboration_RosenbauerLab/DMR calling/HSC_vs_CMOP/dmltest/"
    saveRDS(dmlTest, file = file.path(out.dir, paste0(Sys.Date(), "_dmltest_", "hsc_vs_cmop_", x, ".rds")))
...
# call DMRs   --------------------------------------------------
  ...
  {  
    ...
    out.dir <- "//icgc/dkfzlsdf/analysis/C010/zimmerms/Collaboration_RosenbauerLab/DMR calling/HSC_vs_CMOP/dmrs/"
    saveRDS(dmrs, file = file.path(out.dir, paste0(Sys.Date(), "_dmrs_", "hsc_vs_cmop_", x, "_0.01.rds")))
...
# save dmrs dataframe
  ...
  out.dir <- "/icgc/dkfzlsdf/analysis/C010/zimmerms/Collaboration_RosenbauerLab/DMR calling/HSC_vs_CMOP/"
  saveRDS(dmrs.all.df, file = file.path(out.dir, paste0(Sys.Date(), "_dmrs_", "hsc_vs_cmop_0.01.rds")))
