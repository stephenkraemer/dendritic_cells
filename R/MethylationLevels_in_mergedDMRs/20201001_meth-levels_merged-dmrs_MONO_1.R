# Methylation levels in merged DC DMRs - Mono ------------------------------------------------------
# Dr. Sina Stäble
# 01.10.2020
...
# make list of all samples and replicates ------------------------------------------------------
  ...
  sample.list <- list(mono_1, mono_2, mono_3)
  
  names(sample.list) <- c("mono_1", "mono_2", "mono_3")
...
# methylation level per replicate per DMR using gUtils package   --------------------------------------------------
  ...
  #saveRDS(meth.all, file = file.path(out.dir, paste0(Sys.Date(), "_meth-matrix_DC-DMRs_allChr_MONO.rds")))
  
  cov.all <- mget(ls(pattern = "^cov_mat.chr")) %>%
    map(~as.data.frame(.x)) %>%
    map_dfr(~dplyr::bind_rows(.x)) 
  #saveRDS(cov.all, file = file.path(out.dir, paste0(Sys.Date(), "_cov-matrix_DC-DMRs_allChr_MONO.rds")))
  
  gr.all <- mget(ls(pattern = "^gr.chr")) %>%
    map(~as.data.frame(.x)) %>%
    map_dfr(~dplyr::bind_rows(.x)) %>%
    makeGRangesFromDataFrame(.)
  #saveRDS(gr.all, file = file.path(out.dir, paste0(Sys.Date(), "_gr-object_DC-DMRs_allChr_MONO.rds")))
  ...
  out.dir <- "/icgc/dkfzlsdf/analysis/C010/zimmerms/Collaboration_RosenbauerLab/DMR calling/meth-levels_merged-dmrs/"
  saveRDS(beta_value_allDMRs.df, file = file.path(out.dir, paste0(Sys.Date(), "_beta-value_DC-DMRs_allChr_MONO.rds")))
  write_delim(beta_value_allDMRs.df, path = file.path(out.dir, paste0(Sys.Date(), "_beta-value_DC-DMRs_allChr_MONO.csv")))
